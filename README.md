# Industrial Object Detector

This project was developed for the Artificial Intelligence course and focuses on building an industrial object detection system from start to finish. 
It includes choosing a use case, collecting and labeling data, training and testing models, and preparing the solution for real word use.
The project also explores improving results and comparing different approaches.

# About the Project

Artificial Intelligence project that focuses on creating an end-to-end industrial object detection system.
The system aims to perform real-time detection using a trained model.

# Features
- Real-time object detection using YOLOv8  
- Custom dataset collection and annotation  
- Model training, evaluation, and comparison  

# Technical Pipeline
1. **Data Collection**  
   Collected images of industrial objects for detection tasks  

2. **Annotation**  
   Labeled dataset using Roboflow  

3. **Model Training**  
   Trained YOLOv8m model using Ultralytics  

4. **Inference & Testing**  
   Evaluated model performance on test data  

# Results
- Achieved reliable detection performance on custom dataset  
- Example outputs are available in the `inference_results` folder  

# Note
The raw dataset is not publicly available due to storage and privacy limitations.  
However, trained model outputs and inference results are provided.

#Technologies Used
- Python  
- YOLOv8 (Ultralytics)  
- Roboflow  
- Google Colab
  
# Status 
Improvements are still being made.
